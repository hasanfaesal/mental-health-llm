...
class ModelArguments:
    ...
    max_seq_length: int = field(
        default=2048,
        metadata={"help": "Maximum sequence length for model input"}
    )
...
class DataArguments:
    ...
    dataset_path: str = field(
        default="mental_health_counseling_conversations/combined_dataset.json",
...
class TrainingArguments:
    ...
    gradient_accumulation_steps: int = field(default=8)  # Increased to maintain effective batch size
    ...
    evaluation_strategy: str = field(default="steps")  # Fixed: was "steps" but needs to match validation data availability
...
def create_trainer(model, tokenizer, datasets: Dict[str, Dataset], training_args: TrainingArguments, model_args: ModelArguments) -> SFTTrainer:
    """Create and configure the SFT trainer"""
    
    logger.info("Initializing SFT Trainer...")
    
    # Adjust training arguments based on whether we have validation data
    has_eval_dataset = "eval" in datasets and datasets["eval"] is not None and len(datasets["eval"]) > 0
...
def format_mental_health_data(examples: Dict, tokenizer, prompt_template: str) -> Dict[str, List[str]]:
    """
...
def load_and_prepare_dataset(data_args: DataArguments, tokenizer, prompt_template: str) -> Dict[str, Dataset]:
    ...
    if data_args.validation_split > 0:
        ...
        logger.info(f"Split dataset: {len(datasets['train'])} train, {len(datasets['eval'])} validation")
    else:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Train Llama-3-8B on Mental Health Dataset")
    parser.add_argument("--model_name", default="unsloth/llama-3-8b-bnb-4bit", help="Model name or path")
    parser.add_argument("--dataset_path", default="mental_health_counseling_conversations/combined_dataset.json", help="Dataset path")
    parser.add_argument("--output_dir", default="./mental_health_llama_model", help="Output directory")
    parser.add_argument("--max_steps", type=int, default=100, help="Maximum training steps (for quick testing)")
    ...
    try:
        ...
        if os.path.exists(training_args.output_dir):
            ...
            if checkpoint_dirs:
                latest_checkpoint = os.path.join(training_args.output_dir, max(checkpoint_dirs))
        ...
        test_prompt = prompt_template.format(
            ...
            assistant_response=""
        ).split("assistant<|end_header_id|>")[0] + "assistant<|end_header_id|>"
